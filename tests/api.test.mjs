...
test('PointJoy real PostgreSQL integration',async t=>{
 ...
 try{
  ...
  await t.test('task lifecycle, permissions, resubmission and concurrent approval award once',async()=>{
   ...
   const spec={title:'阅读30分钟',description:'完成阅读后填写收获',points:20};
   assert.equal((await req(1,prefix+'/routines',spec)).status,403);
   const task=await ok(0,prefix+'/routines',spec);
   const claim=await ok(1,prefix+'/routines/'+task.id+'/claim',{});
   assert.equal((await req(1,prefix+'/routines/'+task.id+'/claim',{})).error.code,'ALREADY_CLAIMED');
   assert.equal((await req(2,prefix+'/daily-entries/'+claim.id+'/submit',{submission:'冒用',expectedVersion:1})).status,403);
   let current=await ok(1,prefix+'/daily-entries/'+claim.id+'/submit',{submission:'读完第一章',expectedVersion:1});
   assert.equal((await req(1,prefix+'/daily-entries/'+claim.id+'/review',{decision:'APPROVE',reason:'',expectedVersion:current.version})).status,403);
   const foreign=await group();assert.equal((await req(0,'/groups/'+foreign+'/daily-entries/'+claim.id+'/review',{decision:'APPROVE',reason:'',expectedVersion:current.version})).status,404);
   const oldVersion=current.version;
   current=await ok(0,prefix+'/daily-entries/'+claim.id+'/review',{decision:'REJECT',reason:'请补充心得',expectedVersion:current.version});
   assert.equal((await ok(1,prefix+'/dashboard')).account.balance,0);
   current=await ok(1,prefix+'/daily-entries/'+claim.id+'/submit',{submission:'补充了阅读心得',expectedVersion:current.version});
   assert.equal((await req(0,prefix+'/daily-entries/'+claim.id+'/review',{decision:'APPROVE',reason:'',expectedVersion:oldVersion})).status,409);
   const body={decision:'APPROVE',reason:'完成得很好',expectedVersion:current.version};
   const results=await Promise.all([req(0,prefix+'/daily-entries/'+claim.id+'/review',body),req(0,prefix+'/daily-entries/'+claim.id+'/review',body)]);
   ...
   let own=await ok(0,prefix+'/routines/'+task.id+'/claim',{});own=await ok(0,prefix+'/daily-entries/'+own.id+'/submit',{submission:'我也完成了',expectedVersion:1});
   assert.equal((await req(0,prefix+'/daily-entries/'+own.id+'/review',{decision:'APPROVE',reason:'',expectedVersion:own.version})).error.code,'SELF_OPERATION_FORBIDDEN');
   let c2=await ok(2,prefix+'/routines/'+task.id+'/claim',{});await ok(0,prefix+'/routines/'+task.id+'/close',{});
   c2=await ok(2,prefix+'/daily-entries/'+c2.id+'/submit',{submission:'已领取仍可提交',expectedVersion:1});
   const capActivity=await ok(0,prefix+'/routines',{...spec,points:2000000000});
   c2=await ok(2,prefix+'/routines/'+capActivity.id+'/claim',{});
   c2=await ok(2,prefix+'/daily-entries/'+c2.id+'/submit',{submission:'测试积分上限',expectedVersion:1});
   await grant(g,m2,1);
   assert.equal((await req(0,prefix+'/daily-entries/'+c2.id+'/review',{decision:'APPROVE',reason:'',expectedVersion:c2.version})).error.code,'BALANCE_LIMIT_EXCEEDED');
   assert.equal((await sql.dailyEntry.findUnique({where:{id:c2.id}})).status,'SUBMITTED');
   const another=await ok(0,prefix+'/routines',spec);await ok(0,prefix+'/routines/'+another.id+'/close',{});assert.equal((await req(1,prefix+'/routines/'+another.id+'/claim',{})).status,409);
  ...
  await t.test('daily periods and limited activity time windows are enforced by server',async()=>{
   ...
   const spec={title:'限时挑战',description:'活动规则',points:5,};
   assert.equal((await req(0,prefix+'/events',spec)).status,400);
   assert.equal((await req(0,prefix+'/events',{...spec,startsAt:iso(now+60000),endsAt:iso(now)})).status,400);
   const future=await ok(0,prefix+'/events',{...spec,startsAt:iso(now+60000),endsAt:iso(now+3600000)});
   assert.equal((await req(1,prefix+'/events/'+future.id+'/claim',{})).error.code,'ACTIVITY_UNAVAILABLE');
   const limited=await ok(0,prefix+'/events',{...spec,startsAt:iso(now-60000),endsAt:iso(now+3600000)});
   const c=await ok(1,prefix+'/events/'+limited.id+'/claim',{});assert.equal(c.period,'ONCE');
   await sql.eventEntry.update({where:{id:c.id},data:{expiresAt:new Date(now-1)}});
   assert.equal((await req(1,prefix+'/event-entries/'+c.id+'/submit',{submission:'迟到',expectedVersion:1})).error.code,'ACTIVITY_EXPIRED');
   await sql.event.update({where:{id:limited.id},data:{endsAt:new Date(now-1000)}});
   assert.equal((await req(0,prefix+'/events/'+limited.id+'/claim',{})).error.code,'ACTIVITY_UNAVAILABLE');
   const live=await ok(0,prefix+'/events',{...spec,startsAt:iso(now-60000),endsAt:iso(now+3600000)});
   let liveEntry=await ok(1,prefix+'/events/'+live.id+'/claim',{});
   assert.equal(liveEntry.awardPoints,5);
   await sql.event.update({where:{id:live.id},data:{points:99}});
   liveEntry=await ok(1,prefix+'/event-entries/'+liveEntry.id+'/submit',{submission:'完成限时活动',expectedVersion:1});
   assert.equal((await req(0,prefix+'/daily-entries/'+liveEntry.id+'/review',{decision:'APPROVE',reason:'',expectedVersion:liveEntry.version})).status,404);
   await ok(0,prefix+'/event-entries/'+liveEntry.id+'/review',{decision:'APPROVE',reason:'',expectedVersion:liveEntry.version});
   assert.equal((await ok(1,prefix+'/dashboard')).account.balance,5);
   const eventHistory=await ok(1,prefix+'/me/participations?kind=LIMITED');assert.ok(eventHistory.items.every(c=>c.kind==='LIMITED'));
   assert.equal((await req(0,prefix+'/activities',spec)).status,404);
   const daily=await ok(0,prefix+'/routines',{title:'日常阅读',description:'每天阅读',points:3});
   const day=await ok(1,prefix+'/routines/'+daily.id+'/claim',{});assert.match(day.period,/^\d{4}-\d{2}-\d{2}$/);assert.equal(new Date(day.expiresAt).getUTCHours(),16);
   await sql.dailyEntry.update({where:{id:day.id},data:{occurrenceDate:new Date('2000-01-01T00:00:00Z'),expiresAt:new Date('2000-01-02T00:00:00+08:00')}});
   const again=await ok(1,prefix+'/routines/'+daily.id+'/claim',{});assert.notEqual(again.id,day.id);
   assert.equal((await req(1,prefix+'/routines/'+daily.id+'/claim',{})).error.code,'ALREADY_CLAIMED');
  });
  await t.test('admin progress separates kinds and dates, includes nonparticipants and does not truncate',async()=>{
   ...
   await sql.dailyRoutine.createMany({data:ids.map(id=>({id,groupId:g,creatorMemberId:owner.id,title:'日常检查',description:'检查',points:1}))});
   const day=new Date(Date.now()+8*3600000).toISOString().slice(0,10);
   await sql.dailyEntry.createMany({data:ids.map(routineId=>({groupId:g,routineId,memberId:member,occurrenceDate:new Date(day+'T00:00:00Z'),awardPoints:1,expiresAt:new Date(Date.now()+86400000)}))});
   const report=await ok(0,prefix+'/activity-progress?kind=DAILY&date='+day);
   const history=await ok(1,prefix+'/me/participations?limit=30');assert.equal(history.items.length,30);assert.equal(history.hasMore,true);assert.ok(history.items.every(c=>c.memberId===member&&c.activity.kind==='DAILY'));
   const next=await ok(1,prefix+'/me/participations?limit=30&cursor='+history.nextCursor);assert.equal(next.items.length,30);assert.ok(next.items.every(c=>!history.items.some(first=>first.id===c.id)));
   assert.equal((await ok(0,prefix+'/me/participations')).items.length,0);
   assert.equal((await req(2,prefix+'/me/participations')).status,404);
