...
### 1.1.3 登录恢复与标题居中（2026-09-20）

启动页恢复本机已保存的会话和家庭，短期访问凭证过期时自动刷新；网络或服务暂时失败保留凭证并支持重试，刷新回包丢失时沿用原尝试 ID。首次使用、主动退出或凭证真正失效时才进入登录；资料与家长 PIN 门禁继续生效。公共顶部栏标题居中，家庭名称可点击切换，家庭页概览名称也提供同一入口，家庭选择页可以返回。

全部工作区类型检查、42 项前端测试、H5 / 微信生产构建、WXSS 扫描通过；H5 模拟接口验证了启动恢复、过期凭证刷新、家庭切换及取消，320 / 375 / 430px 下共 18 项标题居中与入口不重叠检查通过。微信官方已成功上传 **1.1.3 开发版本**，完整包 **755160 字节**。开发版上传不等于提交审核或正式发布，微信真机体验仍需扫码确认。

使用 `ubuntu@pointjoy.jinjiazh.com` 完成常规更新：11:21 CST 的部署前加密备份成功，5 个迁移文件与数据库记录校验和一致，无新增迁移。新 API 运行包与产品站点于 11:24 CST 切换至 `/opt/pointjoy/releases/20260920-v1.1.3/{api,site}`，原入口路径改为符号链接；上一版代码保留在 `/opt/pointjoy/rollback/20260920-v1.1.3`。现有数据库、私有文件、环境配置与恢复日志保持原位。站点说明同步首次登录和后续自动恢复行为。

上线后 API / worker 均为 `active/running`、重启计数为 0，备份定时器正常；HTTPS 健康检查返回 200 / `ok`，API 自身版本仍为 `1.0.0`，首页与隐私页内容校验一致，匿名会话检查返回 401。发布包 SHA-256：`fabf10d2379d544159a6d481d83da91232f726562e57f749a29d2529954ebd9d`。服务端打包脚本同时禁用 macOS 扩展属性与 AppleDouble 文件，避免 Linux 解包出现多余迁移文件。

本次证据保存在 Git 忽略的 `work/release-1.1.3/`：`server-prepare.log`、`server-deploy.log`、`wechat-upload.log`、`wechat-preview.log`；微信结构化返回仍保存为 `work/wechat-upload-result.json` 与 `work/wechat-preview-result.json`。
