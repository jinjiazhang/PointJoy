...
process.on('message', async (input: any) => {
  try {
    const original = sharp(input.inputPath, {
    ...
    if (input.purpose !== 'COMPLETION_EVIDENCE') {
      ...
      const crop = input.crop || {
        x: Math.floor((width - Math.min(width, height)) / 2),
        y: Math.floor((height - Math.min(width, height)) / 2),
      ...
      image = image.resize({ width: 1600, height: 1600, fit: 'inside', withoutEnlargement: true });
    }
    ...
    await unlink(input.outputPath).catch(() => {});
    process.send?.({
      ok: false,
      code: error.message === 'INVALID_CROP' ? 'INVALID_CROP' : 'INVALID_IMAGE',
    });
  }
