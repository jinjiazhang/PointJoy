...
export async function ensureChildAccount(db: Db, input: { familyId: string; childId: string }) {
  await db.query(
    'INSERT INTO point_accounts(family_id,child_id) VALUES($1,$2) ON CONFLICT(family_id,child_id) DO NOTHING',
    [input.familyId, input.childId],
  );
}
export async function getAccount(db: Db, familyId: string, childId: string, lock = false) {
  const account = await maybe<any>(
    db,
    `SELECT * FROM point_accounts WHERE family_id=$1 AND child_id=$2${lock ? ' FOR UPDATE' : ''}`,
    [familyId, childId],
  );
  ...
  return {
    ...accountView(account),
    frozen: !!incident,
    freezeReason: incident ? 'RECONCILIATION_REQUIRED' : null,
  };
}
export async function financialIncident(db: Db, familyId: string, childId?: string) {
  return maybe<any>(
    db,
    `SELECT id,code FROM reconcile_incidents WHERE status='OPEN'
    AND (family_id IS NULL OR family_id=$1) AND (child_id IS NULL OR child_id=$2) ORDER BY created_at,id LIMIT 1`,
...
export async function movePoints(
  ...
  if (input.expectedAccountVersion !== undefined) {
    assertVersion(account.version, input.expectedAccountVersion);
  }
  const deltas: Record<Movement, [number, number]> = {
    ACTIVITY_AWARD: [input.points, 0],
    PRAISE: [input.points, 0],
    AWARD_REVERSAL: [-input.points, 0],
    ORDER_HOLD: [-input.points, input.points],
    ORDER_CAPTURE: [0, -input.points],
    ORDER_RELEASE: [input.points, -input.points],
    ORDER_REFUND: [input.points, 0],
  };
  ...
  if (['ACTIVITY_AWARD', 'PRAISE'].includes(input.type)) {
    ...
    must(
      account.availablePoints + account.heldPoints + input.points <= 1000000,
      'GRANT_CAP_EXCEEDED',
      '新增积分达到上限，请先兑换使用',
    );
  }
  must(
    account.availablePoints + a >= 0,
    input.type === 'AWARD_REVERSAL' ? 'REVERSAL_INSUFFICIENT_AVAILABLE' : 'INSUFFICIENT_POINTS',
    '可用积分不足',
  );
  must(account.heldPoints + h >= 0, 'INVALID_STATE', '预留积分异常，请联系管理员核对');
  must(
    Number.isSafeInteger(account.availablePoints + a) &&
      Number.isSafeInteger(account.heldPoints + h),
    'INVALID_STATE',
    '积分数值超过安全范围',
  );
  const updated = await one<any>(
    db,
    'UPDATE point_accounts SET available_points=available_points+$3,held_points=held_points+$4,version=version+1,updated_at=clock_timestamp() WHERE family_id=$1 AND child_id=$2 RETURNING *',
    [input.familyId, input.childId, a, h],
  );
  const entry = await one<any>(
    db,
    `INSERT INTO point_ledger(family_id,child_id,type,available_delta,held_delta,available_after,held_after,account_version,source_type,source_id,actor_user_id,actor_membership_id,actor_mode,reason,reversal_of_id)
  ...
  return {
    account: { ...accountView(updated), frozen: false, freezeReason: null },
    ledgerEntry: entry,
  };
}
export async function registerPoints(app: FastifyInstance, services: Services) {
  ...
  app.get('/families/:familyId/children/:childId/ledger', async (request) => {
    ...
    const result = await rows<any>(
      ...
      `SELECT l.*, COALESCE(u.display_name,'已删除账号') AS operator_name FROM point_ledger l LEFT JOIN users u ON u.id=l.actor_user_id
    ...
    return ok(
      request,
      page.result(
        result.map((r) => ({ ...r, operatorSummary: { name: r.operatorName, mode: r.actorMode } })),
      ),
    );
  });
  app.post('/families/:familyId/children/:childId/praises', async (request) => {
    ...
    return services.mutate(request, { familyId, childId, guardian: true }, async (db, actor) => {
      ...
      await audit(db, actor, 'PRAISE', {
        familyId,
        childId,
        sourceId: result.ledgerEntry.id,
  ...
  app.post('/families/:familyId/ledger/:entryId/reversal', async (request) => {
    const familyId = idParam(request, 'familyId');
    const entryId = idParam(request, 'entryId');
    const body = z
      .object({ reason, expectedAccountVersion: z.number().int().nonnegative().safe() })
      .strict()
      .parse(request.body);
    return services.mutate(request, { familyId, guardian: true }, async (db, actor) => {
      const original = await maybe<any>(
        db,
        'SELECT * FROM point_ledger WHERE family_id=$1 AND id=$2',
        [familyId, entryId],
      );
      must(original, 'RESOURCE_NOT_FOUND', '积分记录不存在', 404);
      must(
        ['ACTIVITY_AWARD', 'PRAISE'].includes(original.type),
        'INVALID_STATE',
        '只能完整撤销原始发奖或表扬',
      );
      await activeChild(db, familyId, original.childId);
      if (original.type === 'ACTIVITY_AWARD') {
        const occurrence = await one<any>(
          db,
          'SELECT * FROM activity_occurrences WHERE family_id=$1 AND id=$2 FOR UPDATE',
          [familyId, original.sourceId],
        );
        must(occurrence.status === 'APPROVED', 'INVALID_STATE', '该完成记录不能撤销');
      }
      await one(db, 'SELECT id FROM point_ledger WHERE family_id=$1 AND id=$2 FOR UPDATE', [
        familyId,
        entryId,
      ]);
      must(
        !(await maybe(db, 'SELECT id FROM point_ledger WHERE reversal_of_id=$1', [entryId])),
        'ALREADY_REVERSED',
        '该积分已撤销',
      );
      const result = await movePoints(db, actor, {
        ...
        type: 'AWARD_REVERSAL',
        points: original.availableDelta,
        sourceType: 'REVERSAL',
        sourceId: entryId,
        reason: body.reason,
      ...
      if (original.type === 'ACTIVITY_AWARD') {
        await db.query(
          "UPDATE activity_occurrences SET status='REVOKED',version=version+1,updated_at=clock_timestamp() WHERE family_id=$1 AND id=$2",
          [familyId, original.sourceId],
        );
        await db.query(
          "INSERT INTO completion_decisions(family_id,occurrence_id,action,reason,operator_membership_id) VALUES($1,$2,'REVOKE',$3,$4)",
          [familyId, original.sourceId, body.reason, actor.membershipId],
        );
      }
      await audit(db, actor, 'AWARD_REVERSAL', {
        familyId,
        childId: original.childId,
        sourceId: entryId,
