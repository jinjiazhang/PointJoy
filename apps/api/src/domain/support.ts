...
export const ids = z
  ...
  .min(1)
  .max(100)
  .refine((a) => new Set(a).size === a.length, '不能重复选择孩子');
export const mediaIds = z
  .array(uuid)
  .max(3)
  .refine((a) => new Set(a).size === a.length, '不能重复选择照片')
  .default([]);
...
export const today = (now = new Date()) =>
  new Date(now.getTime() + 8 * HOUR).toISOString().slice(0, 10);
...
export const addDays = (date: string, amount: number) =>
  today(new Date(dayStart(date).getTime() + amount * DAY));
...
export function assertVersion(actual: number, expected: number) {
  if (actual !== expected) {
    throw new AppError(409, 'VERSION_CONFLICT', '数据已更新，请查看最新内容', {
...
export function target(actor: Actor, familyId: string, childId: string) {
  if (actor.mode !== 'GUARDIAN' && actor.mode !== 'CHILD') {
    throw new AppError(403, 'FORBIDDEN', '请进入有效家庭');
  }
  if (actor.mode === 'CHILD' && (actor.familyId !== familyId || actor.childId !== childId)) {
    throw new AppError(404, 'RESOURCE_NOT_FOUND', '记录不存在');
  }
}
export async function activeChild(db: Db, familyId: string, childId: string) {
  const child = await maybe<any>(db, 'SELECT * FROM child_profiles WHERE family_id=$1 AND id=$2', [
    familyId,
    childId,
  ]);
  if (!child) {
    throw new AppError(404, 'RESOURCE_NOT_FOUND', '孩子档案不存在');
  }
...
export async function validateChildren(db: Db, familyId: string, childIds: string[]) {
  const children = await rows<any>(
    db,
    "SELECT * FROM child_profiles WHERE family_id=$1 AND id=ANY($2::uuid[]) AND status='ACTIVE'",
    [familyId, childIds],
  );
...
export function params(request: any) {
  return request.params as Record<string, string>;
}
export function idParam(request: any, key: string) {
  return uuid.parse(params(request)[key]);
}
...
export function paging(query: any, scope: unknown) {
  ...
  if (cursor) {
    try {
      ...
      throw new AppError(400, 'VALIDATION_ERROR', '分页位置无效，请重新加载');
    }
  }
  return {
    limit,
    ...
    result<T extends { createdAt: string; id: string }>(records: T[]) {
      ...
      return {
        ...
        nextCursor:
          hasMore && last
            ? Buffer.from(
                JSON.stringify({ scope: signature, createdAt: last.createdAt, id: last.id }),
              ).toString('base64url')
            : null,
      };
    },
...
export function range(query: any, maxDays = 366) {
  ...
  must(
    start <= end && dayStart(end).getTime() - dayStart(start).getTime() < maxDays * DAY,
    'VALIDATION_ERROR',
    `请选择不超过${maxDays}天的日期范围`,
    400,
  );
...
export function enumFilter(value: unknown, allowed: readonly string[]): string | null {
  ...
  if (typeof value !== 'string' || !allowed.includes(value)) {
    throw new AppError(400, 'VALIDATION_ERROR', '筛选条件无效');
  }
  return value;
}
export function accountView(account: any) {
  return {
    ...account,
    totalHeldPoints: account.availablePoints + account.heldPoints,
    grantBlockedByCap: account.availablePoints + account.heldPoints >= 1000000,
  };
}
