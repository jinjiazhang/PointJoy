...
export async function audit(db:Db,actor:Actor|null,action:string,input:{familyId?:string|null;childId?:string|null;sourceId?:string|null;details?:unknown}={}){
  ...
  await outbox(db,`domain:${randomUUID()}`,'DOMAIN_CHANGED',{
    action,familyId:input.familyId??actor?.familyId??null,childId:input.childId??null,sourceId:input.sourceId??null,
  });
}
