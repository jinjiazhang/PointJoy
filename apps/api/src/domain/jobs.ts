...
async function collectIssues(db: Db) {
  ...
  for (const [code, sql] of Object.entries(checks)) {
    for (const detail of await rows<any>(db, sql)) {
  ...
  return transaction(
    ...
    async (db) => {
      ...
      const counts = await one<any>(
        db,
        'SELECT (SELECT COUNT(*)::int FROM point_accounts) accounts,(SELECT COUNT(*)::int FROM point_ledger) ledger_entries,(SELECT COUNT(*)::int FROM redemption_orders) orders,(SELECT COUNT(*)::int FROM activity_occurrences) occurrences',
      );
      for (const issue of issues) {
        await db.query(
          "INSERT INTO reconcile_incidents(family_id,child_id,code,details) SELECT $1,$2,$3,$4 WHERE NOT EXISTS(SELECT 1 FROM reconcile_incidents WHERE status='OPEN' AND family_id IS NOT DISTINCT FROM $1 AND child_id IS NOT DISTINCT FROM $2 AND code=$3)",
          [issue.familyId ?? null, issue.childId ?? null, issue.code, JSON.stringify(issue)],
        );
      }
      const report = await one<any>(
        db,
        'INSERT INTO domain_reconciliation_reports(status,issues,counts) VALUES($1,$2,$3) RETURNING *',
        [issues.length ? 'FAIL' : 'PASS', JSON.stringify(issues), JSON.stringify(counts)],
      );
      return report;
    },
    'REPEATABLE READ',
...
export async function resolveIncidents(
  services: Services,
  input: { familyId: string; childId?: string; operator: string; reason: string },
) {
  if (!input.operator.trim() || !input.reason.trim()) {
    throw new AppError(400, 'VALIDATION_ERROR', '运维操作必须记录操作人和修复原因');
  }
  return transaction(services.pool, async (db) => {
    await one(db, 'SELECT id FROM families WHERE id=$1 FOR UPDATE', [input.familyId]);
    const remaining = (await collectIssues(db)).filter(
      (x) =>
        (!x.familyId || x.familyId === input.familyId) &&
        (!input.childId || !x.childId || x.childId === input.childId),
    );
    if (remaining.length) {
      throw new AppError(409, 'RECONCILIATION_FAILED', '仍存在账务不一致，不能解除冻结', {
        issues: remaining,
      });
    }
    const resolved = await rows<any>(
      db,
      `UPDATE reconcile_incidents SET status='RESOLVED',resolved_at=clock_timestamp(),
      details=details||jsonb_build_object('resolution',jsonb_build_object('operator',$3::text,'reason',$4::text))
      WHERE status='OPEN' AND family_id=$1 AND ($2::uuid IS NULL OR child_id=$2) RETURNING id,code,child_id`,
      [input.familyId, input.childId ?? null, input.operator.trim(), input.reason.trim()],
    );
    await audit(db, null, 'RECONCILIATION_RESOLVE', {
      familyId: input.familyId,
      childId: input.childId ?? null,
      details: {
        operator: input.operator.trim(),
        reason: input.reason.trim(),
        incidentIds: resolved.map((x) => x.id),
      },
    });
...
async function oncePerDay(services: Services, type: string, work: () => Promise<void>) {
  ...
  const claimed = await rows(
    services.pool,
    `INSERT INTO domain_job_runs(job_key,status,lease_until) VALUES($1,'RUNNING',clock_timestamp()+interval '15 minutes') ON CONFLICT(job_key) DO UPDATE SET status='RUNNING',lease_until=clock_timestamp()+interval '15 minutes',attempts=domain_job_runs.attempts+1 WHERE domain_job_runs.status<>'DONE' AND domain_job_runs.lease_until<clock_timestamp() RETURNING job_key`,
    [key],
  );
  if (!claimed.length) {
    return;
  ...
  try {
    ...
    await services.pool.query(
      "UPDATE domain_job_runs SET status='DONE',completed_at=clock_timestamp() WHERE job_key=$1",
      [key],
    );
  } catch (error) {
    await services.pool.query(
      "UPDATE domain_job_runs SET status='FAILED',lease_until=clock_timestamp() WHERE job_key=$1",
      [key],
    );
    throw error;
  }
...
export async function runDomainJobs(services: Services) {
  ...
  await oncePerDay(services, 'materialize', async () => {
    const families = await rows<any>(
      services.pool,
      "SELECT id FROM families WHERE status='ACTIVE' ORDER BY id",
    );
    for (const family of families) {
      await materialize(services.pool as any, { familyId: family.id, from: today(), to: today() });
    }
  });
  await oncePerDay(services, 'reconcile', async () => {
