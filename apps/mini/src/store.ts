...
export const useApp=defineStore('app',{
 ...
 actions:{
  async init(){if(uni.getStorageSync('pj-data-version')!=='daily-events-v1'){for(const key of uni.getStorageInfoSync().keys)if(key.startsWith('pj-'))uni.removeStorageSync(key);uni.setStorageSync('pj-data-version','daily-events-v1');}this.authMode=(await api('/auth/config')).mode;this.users=this.authMode==='demo'?await api('/auth/demo-users'):[];if(uni.getStorageSync('pj-token')){try{this.user=await api('/me');await this.loadGroups();}catch{uni.removeStorageSync('pj-token');this.user=null;}}},
