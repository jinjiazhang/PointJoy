import { loadAuth, saveAuth, clearAuth, refreshAttemptId, uuid } from './vault'
...
async function refresh(){if(refreshPromise)return refreshPromise;const auth=loadAuth();if(!auth?.refreshToken)throw new ApiError('SESSION_EXPIRED','请重新微信登录',{},401);refreshPromise=(async()=>{try{const result=await transport<Tokens>('POST','/auth/refresh',{refreshToken:auth.refreshToken,refreshAttemptId:refreshAttemptId()});saveAuth(result)}catch(e){if(e instanceof ApiError&&e.status===401)clearAuth();throw e}finally{refreshPromise=null}})();return refreshPromise}
export async function request<T>(method:HttpMethod,path:string,body?:unknown,headers:Record<string,string>={},retry=true):Promise<T>{try{return await transport<T>(method,path,body,headers)}catch(e){if(e instanceof ApiError&&e.status===401&&retry&&(path==='/auth/session'||!path.startsWith('/auth/'))){await refresh();return request<T>(method,path,body,headers,false)}throw e}}
