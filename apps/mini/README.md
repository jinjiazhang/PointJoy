...
基于 `docs` 的 P0 实现。uni-app + Vue 3 + TypeScript + Pinia，32 个注册页面，生产仅微信登录，资料门禁要求服务端处理成功的真实上传头像和昵称。

1.1.2 使用 iOS 27 启发的顶部玻璃质感导航、浅色内容卡片及分组表单。顶部去掉独立的“积乐圈”品牌行和正文重复的家庭名称、大标题：家长主标签同一行显示“家庭名称 ▾ + 当前页面短标题”，点击家庭名称切换家庭；详情和表单显示返回图标与短标题。孩子主标签同一行保留“家长入口”或“我的账号”与标题；前者用于共用设备 PIN 验证退出，后者用于独立账号。孩子 API 未提供家庭名称，界面不为显示名称扩大访问权限。长家庭名省略显示，并为微信胶囊预留空间。

底部延续 1.1.1 的全宽白底、贴底固定标签栏，顶部细分隔线，不设外边距、圆角或阴影；选中图标与文字为蓝色，白底延续至底部安全区。实现范围见 [界面说明](../../docs/小程序界面.md)。共享图标为自制 PNG，可运行 `node scripts/generate-ui-icons.mjs` 重新生成（从仓库根目录执行）。兑换浮层内显示操作失败原因；无上一页时，返回当前身份的首页；积分账本默认直接加载积分明细。
