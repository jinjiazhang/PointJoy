...
function intent(changes: Partial<UploadIntent> = {}): UploadIntent {
  return {
    mediaId: MEDIA_ID,
    uploadUrl: `${apiBase}/media/uploads/${PRIVATE_TOKEN}`,
    uploadMethod: 'POST',
    formFieldName: 'file',
    formData: {},
    requiredHeaders: {},
...
function item(): UploadItem {
  return {
    ...
    name: 'private-family-name.png',
    status: 'UPLOADING',
    progress: 0,
    mediaId: MEDIA_ID,
  };
}
function envelope(mediaId = MEDIA_ID) {
  return { data: { uploaded: true, mediaId } };
}
...
function contentType(request: any): string {
  const header = Object.entries(request.header || {}).find(
...
beforeEach(() => {
  ...
  uploadHandler = (options) =>
    options.success({ statusCode: 200, data: JSON.stringify(envelope()) });
  installFileSystem();
});

for (const [mime, bytes, extension] of [
  ['image/png', png, 'png'],
  ['image/jpeg', jpeg, 'jpe?g'],
] as const) {
  test(`multipart round-trips ${mime} bytes with a real form-data parser`, async () => {
    ...
    assert.ok(
      raw.subarray(-boundary.length - 8).equals(Buffer.from(`\r\n--${boundary}--\r\n`)),
      'closing delimiter is complete and CRLF terminated',
    );
    const headerEnd = raw.indexOf('\r\n\r\n');
    assert.ok(headerEnd > 0);
    assert.match(
      raw.subarray(0, headerEnd).toString('ascii'),
...
test('WeChat reads binary without encoding and sends POST ArrayBuffer through request', async () => {
  ...
  await transport.sendImageFile(
    intent({ requiredHeaders: { 'X-Upload-Test': 'preserved' } }),
    image,
    'image/png',
    png.byteLength,
  );
...
test('exactly 10 MiB is accepted and a larger advertised file is rejected before transfer', async () => {
  ...
  await assert.rejects(
    transport.sendImageFile(intent(), item(), 'image/png', MAX_BYTES + 1),
    /10\s*MiB|过大|超过/,
  );
...
test('actual oversized bytes cannot bypass the size gate using a smaller advertised size', async () => {
  ...
  await assert.rejects(
    transport.sendImageFile(intent(), item(), 'image/png', png.byteLength),
    assertPrivateError,
  );
  assert.equal(requests.length + uploads.length, 0);
});
...
for (const [label, url] of [
  [
    'different origin',
    () => `https://uploads.example.invalid/api/v1/media/uploads/${PRIVATE_TOKEN}`,
  ],
  ...
  test(`rejects ${label} before transfer`, async () => {
    await assert.rejects(
      transport.sendImageFile(intent({ uploadUrl: url() }), item(), 'image/png', png.byteLength),
      assertPrivateError,
    );
    assert.equal(requests.length + uploads.length, 0);
  });
...
test('expired intent returns an actionable 410 error without exposing upload URL', async () => {
  requestHandler = (options) =>
    options.success({
      statusCode: 410,
      data: { error: { message: `${PRIVATE_TOKEN} ${PRIVATE_PATH}` } },
    });
  await assert.rejects(
    transport.sendImageFile(intent(), item(), 'image/png', png.byteLength),
    (error) => {
      assertPrivateError(error);
      assert.match((error as Error).message, /过期|失效/);
      return true;
    },
  );
  assert.equal(requests.length, 1);
...
test('network failure suggests retry without automatically repeating transfer or leaking SDK errors', async () => {
  requestHandler = (options) =>
    options.fail({ errMsg: `request:fail ${intent().uploadUrl} ${PRIVATE_PATH}` });
  await assert.rejects(
    transport.sendImageFile(intent(), item(), 'image/png', png.byteLength),
    (error) => {
      assertPrivateError(error);
      assert.equal((error as { code: string }).code, 'MEDIA_NETWORK_FAILED');
      assert.match((error as Error).message, /网络|中断/);
      assert.match((error as Error).message, /重试|重新/);
      return true;
    },
  );
  assert.equal(requests.length, 1);
...
test('request timeout is distinguishable and does not repeat a potentially accepted upload', async () => {
  requestHandler = (options) =>
    options.fail({ errMsg: `request:fail timeout ${intent().uploadUrl}` });
  await assert.rejects(
    transport.sendImageFile(intent(), item(), 'image/png', png.byteLength),
    (error) => {
      assertPrivateError(error);
...
test('H5 fallback also validates response media ID and never retries automatically', async () => {
  delete uniMock.getFileSystemManager;
  uploadHandler = (options) =>
    options.success({ statusCode: 200, data: JSON.stringify(envelope('wrong-media')) });
  await assert.rejects(
    transport.sendImageFile(intent(), item(), 'image/png', png.byteLength),
    assertPrivateError,
  );
