...
更新：2026-09-20 · 状态：当前架构设计；容量指标需实测验证。

本文描述当前系统的模块与工程约束。业务规则以 [00-规则基线](00-规则基线.md) 为共同契约，接口见 [06-技术文档](06-技术文档.md)。实现范围与运行方式见 [项目说明](../README.md)。
...
客户端使用 uni-app、Vue 3、TypeScript、Pinia；服务端使用 Fastify 5、TypeScript、Zod 和 pg 参数化 SQL；数据库使用 PostgreSQL 16。依赖版本由 package.json 与 package-lock.json 锁定，数据库结构通过 apps/api/migrations 中的 SQL 迁移维护。P0 账号头像和孩子档案头像必须真实上传，完成提交可附照片；私有对象存储保存处理后的头像、完成照片和临时导出包。奖励封面使用内置插画，自定义奖励封面属于 P1。
...
PIN 采用带独立盐的抗离线猜测密码哈希，算法及成本在实现时按安全库与设备容量选定；不得明文或可逆加密存储。账号维度 5 次连续失败锁定 15 分钟，同时按会话与来源限流，不以重装清零。失败返回剩余等待时间，不泄露 PIN 内容；并发失败计数原子更新。PIN 不是实名或监护资格认证，也不承诺防止掌控设备、调试环境、恢复码或家长凭据的人攻击。
...
- 流水、决策、审计应用账号只允许 INSERT/SELECT；变更数据库结构与隐私删除使用隔离的受审计运行身份。约束、部分索引、延迟外键和权限使用受版本管理的 SQL 维护。
...
身份、家庭、媒体与隐私模块位于 `apps/api/src` 对应目录，计划、积分和奖励位于 `apps/api/src/domain`，事务、幂等与恢复保护位于 `apps/api/src/common`。数据变更遵循 SQL 迁移、备份、对账与恢复流程；外部数据导入边界见验收文档。
